...
//
// 用法:
//   OPENAI_API_KEY=sk-... node script/tts.mjs [檔案或資料夾...] [選項]
//
...
//   --limit <n>        每批最大字元數 (預設 3500,須 < 4096)
//   --concurrency <n>  同檔內片段的併發請求數 (預設 4)
//   --force            mp3 已存在也重新生成 (預設跳過)
//   --dry-run          只印出分批計畫,不呼叫 API、不寫檔
//
// 範例:
//   node script/tts.mjs                                    # 全部講稿
//   node script/tts.mjs book                               # 只做 book/
//   node script/tts.mjs book/01-prompt-chaining.transcript.md
//   node script/tts.mjs --voice nova --model tts-1-hd book-lite
...
function printHelp() {
  // 檔頭註解即說明,這裡印出精簡版
  console.log(`用法: node script/tts.mjs [檔案或資料夾...] [--voice n] [--model n] [--limit n] [--concurrency n] [--force] [--dry-run]
